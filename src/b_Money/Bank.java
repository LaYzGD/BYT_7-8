...
public class Bank {
	...
	public void openAccount(String accountid) throws AccountExistsException {
		if (accountlist.containsKey(accountid)) {
			...
			//should put a new account into the list instead of get
	...
	public void deposit(String accountid, Money money) throws AccountDoesNotExistException {
		//we should throw an exception in case if account does not exist
		if (!accountlist.containsKey(accountid)) {
	...
	public void withdraw(String accountid, Money money) throws AccountDoesNotExistException {
		if (!accountlist.containsKey(accountid)) {
			...
			Money moneyToWithdraw = money;
			if(money.getAmount() > account.getBalance().getAmount()){
				moneyToWithdraw = new Money(account.getBalance().getAmount(), money.getCurrency());
			}
			//should be "withdraw" instead of deposit
	...
	public void transfer(String fromaccount, Bank tobank, String toaccount, Money amount) throws AccountDoesNotExistException {
		...
		if(amount.getAmount() > fromAccount.getBalance().getAmount()){
			_amount = new Money(fromAccount.getBalance().getAmount(), amount.getCurrency());
		}
